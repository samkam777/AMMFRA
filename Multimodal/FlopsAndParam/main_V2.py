...
def image_feature(img_feature_df, movie_history, sequence):
    ...
    # img_feature_df = pd.read_csv(data_path + "/img_feature/img_feature_1024_V2.csv")
    # print(eval(str(img_feature_df.loc[img_feature_df['image_name'] == total_img_name, 'image_feature'].values[0][1:-1])))
    ...
    img_f = eval(str(img_feature_df.loc[img_feature_df['image_name'] == total_img_name, 'image_feature'].values[0][1:-1]))
    ...
    if sequence:
        ...
        for idx, item in enumerate(movie_history[1:]):
            ...
            img = eval(str(img_feature_df.loc[img_feature_df['image_name'] == item.item(), 'image_feature'].values[0][1:-1]))
...
def txt_feature(txt_feature_tensor, movie_history, sequence):
    ...
    txt_f = eval(
        str(txt_feature_tensor.loc[txt_feature_tensor['txt_name'] == total_txt_name, 'txt_feature'].values[0][1:-1]))
    ...
    if sequence:
        ...
        for idx, item in enumerate(movie_history[1:]):
            ...
            txt = eval(str(
                txt_feature_tensor.loc[txt_feature_tensor['txt_name'] == item.item(), 'txt_feature'].values[0][1:-1]))
...
def genres_feature(genres_feature_df, movie_history, sequence):
    ...
    genres_feature_df = pd.read_csv(data_path + "/movies_genres.csv", sep=",")
    movie_genres_f_str = genres_feature_df.loc[genres_feature_df["itemid"] == total_movie_name, "encoded_cat"].values[0][1:-1].split()
    movie_genres_f = [float(x) for x in movie_genres_f_str]
    # print(movie_genres_f_str)
    ...
    if sequence:
        ...
        for idx, item in enumerate(movie_history[1:]):
            ...
            genres_str = genres_feature_df.loc[genres_feature_df["itemid"] == item.item(), "encoded_cat"].values[0][1:-1].split()
            genres = [float(x) for x in genres_str]
...
class BST(nn.Module):
    def __init__(self, users, ratings, movies, genres, device, args=None):
        ...
        # print(self.users.shape) # (6040, 5)
        # print(self.ratings.shape) # (6040, 5)
        # print(self.movies.shape) # (6040, 5)
        # print(self.genres.shape) # (6040, 5)
        ...
        total_emd_param = 2*movie_id_param+user_id_param
        print(f"BST embedding 参数量 : {total_emd_param}")
    ...
    def encode_input(self, inputs):
        ...
        #     print("input datasize {} : {}".format(index, i.shape))
        # print("===============================")
        # for i in inputs:
        #     print(i.shape)
        # print("===============================")
        ...
        movie_history = movie_history.to(torch.long)
        ...
        # print("movie_history_emb.shape: {}".format(movie_history_emb.shape))
        # print("target_movie_emb.shape: {}".format(target_movie_emb.shape))
        transformer_features = torch.cat((movie_history_emb, target_movie_emb), dim=1) # 128, 8, 136
        # print(transformer_features.shape)

        # print("target_movie_emb : {}".format(target_movie_emb.shape))           # torch.Size([128, 1, 136])
        # print("user_id : {}".format(user_id.shape))                             # torch.Size([128, 78])
        # print("movie_history_id_emb : {}".format(movie_history_id_emb.shape))   # torch.Size([128, 3, 63])
    ...
    def forward(self, user_id, movie_history, target_movie_id, movie_history_ratings, target_movie_rating, sex, age_group, occupation, movie_history_genres, target_movie_genres, target_movie_id_image_feature, target_movie_id_txt_feature):
        x = user_id, movie_history, target_movie_id, movie_history_ratings, target_movie_rating, sex, age_group, occupation, movie_history_genres, target_movie_genres, target_movie_id_image_feature, target_movie_id_txt_feature
...
def train(args):
    ...
    for alg in ["NeuMF", "BST", "Item2Vec", "AutoInt", "DCNV2"]:
        args.algorithm = alg
        ...
        if args.algorithm == "BST":
            ...
            input_datasize = [(args.batch_size,), (args.batch_size,3), (args.batch_size,), 
                (args.batch_size,3), (args.batch_size,), (args.batch_size,), (args.batch_size,), 
                (args.batch_size,), (args.batch_size, 3,  73), (args.batch_size,  73), (args.batch_size, 1024), (args.batch_size, 768)]
            
            profile_x = [torch.rand(*in_size).type(dtype) for in_size in input_datasize]
            macs, params = profile(model, inputs=profile_x)
            embedding_macs =    [[128, 1, 136],
                                [128, 78],
                                [128, 3, 63]]
            embedding_macs_sum = 0
            for emb in embedding_macs:
                embedding_macs_sum += emb[0] * emb[1]
            print(f"BST embedding FLOPs：{embedding_macs_sum * 2}")
            print(f"BST模型的 FLOPs：{macs*2}")
            print(f"BST模型的参数量：{params}")
            print()
            # input_datasize = [(), (3,), (), 
            #     (3,), (1,), (1,), (1,), 
            #     (1,), (3,  73,), (73, ), (1024,), (768,)]
            
            # print(input_datasize)
            # input_datasize = (args.batch_size, 
            #                 (args.batch_size,3), 
            #                 args.batch_size, 
            #                 (args.batch_size,3), 
            #                 args.batch_size, 
            ...
            #                 args.batch_size, 
            #                 args.batch_size, 
            #                 (args.batch_size, 3,  73), 
            #                 (args.batch_size, 1024), 
            #                 (args.batch_size, 768))
            # input_datasize = {
            #     'user_id': (args.batch_size,),
            #     'movie_history': (args.batch_size, 3),
            #     'target_movie_id': (args.batch_size,),
            #     'movie_history_ratings': (args.batch_size, 3),
            #     'target_movie_rating': (args.batch_size,),
            #     'sex': (args.batch_size,),
            #     'age_group': (args.batch_size,),
            #     'occupation': (args.batch_size,),
            #     'movie_history_genres': (args.batch_size, 3, 73),
            #     'target_movie_genres': (args.batch_size, 1024),
            #     'target_movie_id_image_feature': (args.batch_size, 768),
            #     'target_movie_id_txt_feature': (args.batch_size, 768),
            # }
            # input_datasize = (
            #     args.batch_size,  # 用户ID的形状
            #     args.batch_size, 3,  # 电影历史的形状
            #     args.batch_size,  # 目标电影ID的形状
            #     args.batch_size, 3,  # 电影历史评分的形状
            #     args.batch_size,  # 目标电影评分的形状
            #     args.batch_size,  # 性别的形状
            #     args.batch_size,  # 年龄组的形状
            #     args.batch_size,  # 职业的形状
            #     args.batch_size, 3, 73,  # 电影历史类型的形状
            #     args.batch_size, 1024,  # 目标电影ID的图像特征形状
            #     args.batch_size, 768,  # 目标电影ID的文本特征形状
            # )

            # 将 input_datasize 中的元组形状参数转换为整数
            # input_datasize = (tuple(map(int, size)) for size in input_datasize)
            # model = model.to(device)
            # print(summary(model,input_datasize, batch_size=128))
            ...
            input_datasize = [(args.batch_size,), (args.batch_size,), (args.batch_size,)]
            
            profile_x = [torch.rand(*in_size).type(dtype) for in_size in input_datasize]
            macs, params = profile(model, inputs=profile_x)

            embedding_macs =[[128, 32],
                            [128, 32],
                            [128, 32],
                            [128, 32]]
            embedding_macs_sum = 0
            for emb in embedding_macs:
                embedding_macs_sum += emb[0] * emb[1]
            print(f"NeuMF embedding FLOPs：{embedding_macs_sum * 2}")
            print(f"NeuMF模型的 FLOPs：{macs*2}")
            print(f"NeuMF模型的参数量：{params}")
            print()
            ...
            embedding_macs =[[128, 100],
                            [128, 100]]
            embedding_macs_sum = 0
            for emb in embedding_macs:
                embedding_macs_sum += emb[0] * emb[1]
            print(f"Item2Vec embedding FLOPs：{embedding_macs_sum * 2}")
            print(f"Item2Vec模型的 FLOPs：{macs*2}")
            print(f"Item2Vec模型的参数量：{params}")
            print()
            ...
            input_datasize = [(args.batch_size,), (args.batch_size,), (args.batch_size,), (args.batch_size,), (args.batch_size,), (args.batch_size,)]
            
            profile_x = [torch.rand(*in_size).type(dtype) for in_size in input_datasize]
            macs, params = profile(model, inputs=profile_x)

            embedding_macs =    [[128, 1],
                                [128, 2],
                                [128, 4],
                                [128, 63],
                                [128, 78]]
            ...
            for emb in embedding_macs:
                embedding_macs_sum += emb[0] * emb[1]
            print(f"AutoInt embedding FLOPs：{embedding_macs_sum * 2}")
            print(f"AutoInt模型的 FLOPs：{macs*2}")
            print(f"AutoInt模型的参数量：{params}")
            print()
            optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr)
            criterion = torch.nn.BCELoss()
            ...
            for emb in embedding_macs:
                embedding_macs_sum += emb[0] * emb[1]
            print(f"DCNV2 embedding FLOPs：{embedding_macs_sum * 2}")
            print(f"DCNV2模型的 FLOPs：{macs*2}")
            print(f"DCNV2模型的参数量：{params}")
            print()
            # optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr)
            # criterion = torch.nn.BCELoss()

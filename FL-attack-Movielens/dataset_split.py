...
def get_data(args):
    ...
    shuffle_l = shuffle_list(n_u, args.n_users, 100)  # 包含0索引，6040个用户，50份，100最小数（随机取）
    print(shuffle_l)
    ...
    local_train_datasets = dataset_split(train_dataset, args.n_users, balance=False, path=data_path + "/new_dataset4/train_data.csv", shuffle_l=shuffle_l)
    local_val_datasets = dataset_split(val_dataset, args.n_users, balance=False, path=data_path + "/new_dataset4/val_data.csv", shuffle_l=shuffle_l)
    local_test_datasets = dataset_split(test_dataset, args.n_users, balance=False, path=data_path + "/new_dataset4/test_data.csv", shuffle_l=shuffle_l)
    ...
    return users, ratings, movies, genres, local_train_datasets, local_val_datasets, local_test_datasets
...
def dataset_split(dataset, n_workers, balance, path, shuffle_l = None):
    ...
    n_user = data_df['user_id'].nunique()     
    # print(n_user)
    userid_g = data_df.groupby('user_id')     
    # print(userid_g)
    # print(userid_g.tail(1))                       
    data_index = userid_g.tail(1).index.tolist()
    data_index.insert(0, 0)                       
...
def shuffle_list(amount, num, min_amount):
    ...
    list2 = generate_sorted_list_with_sum(0, amount - 35, num - 1, amount - 35)

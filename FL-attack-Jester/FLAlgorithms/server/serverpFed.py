...
from FLAlgorithms.users.user_attack import UserAttack
...
class pFed(Server):
    def __init__(self, device, args, train_dataset, val_dataset, test_dataset, model, running_time, hyper_param, writer):
        ...
        for i in range(self.n_users):
            ...
            normal_users = i
        for j in range(self.n_attack_data):
            attack_user = UserAttack(device, args, model, j+normal_users+1, running_time, hyper_param)
            self.users.append(attack_user)
    ...
    def train(self):
        ...
        for global_epoch in range(self.global_epochs):
            ...
            self.selected_users = self.select_users(global_epoch, int((self.n_users+self.n_attack_data) * self.n_select_users_rate))
            # 不设置抽样
            # self.selected_users = self.select_users(global_epoch, self.n_users+self.n_attack_data)

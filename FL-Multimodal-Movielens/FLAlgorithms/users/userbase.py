...
class User:
    def __init__(self, device, args, model, train_dataset, val_dataset, test_dataset, numeric_id, running_time, hyper_param, criterion):
        self.device = device
        ...
        print("n_train_data_samples :{}".format(self.n_train_data_samples))
        self.id = numeric_id
    ...
    def global_test(self):
        ...
        for datas in self.val_data:
            ...
            with torch.no_grad():
                ...
                # print(target_movie_rating)

                loss = self.criterion(out, target_movie_rating)
                test_losses.append(loss.item())

                # auc = roc_auc_score(target_movie_rating.cpu().detach().numpy(), out.cpu().detach().numpy())
                auc, f1, acc, recall, precision = self.compute_metrics(target_movie_rating.cpu().detach().numpy(), out.cpu().detach().numpy())
                test_auc.append(auc)
                test_f1.append(f1)
                test_acc.append(acc)
...
###################### user evaluate ######################
    ...
    def user_persionalized_evaluate(self, epoch, train_loss):
        ...
        print("user: {}\tloss:{:.4f}\tauc: {:.4f}\tf1:{:.4f}\tacc:{:.4f}\trecall:{:.4f}\tprecision:{:.4f}".format(self.id, losses, auc, f1, acc, recall, precision))

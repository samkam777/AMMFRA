...
class Item2Vec(nn.Module):
    def __init__(self):
        ...
        shared_embedding_param = (self.args['vocabulary_size']) * (self.args['embedding_dim'])
        print(f"Item2Vec embedding 参数量 : {shared_embedding_param*2}")


    def forward(self, target_i, context_j, label):
        # target_i, context_j, label = input_data
        target_i = target_i.to(torch.long)
        context_j = context_j.to(torch.long)
        ...
        # print("target_emb : {}".format(target_emb.shape))   # torch.Size([128, 100])
        # print("context_emb : {}".format(context_emb.shape)) # torch.Size([128, 100])

...
class Server:
    def __init__(self, device, args, train_dataset, val_dataset, test_dataset, model, running_time, hyper_param, writer):
        ...
        print("n_users :{}".format(self.n_users))
        ...
        # attack
        # self.n_attack_data = int(self.n_users * args.attack_rate)
        self.n_attack_data = int(args.attack_rate / (1-args.attack_rate) * self.n_users)
        print("n_attack_data :{}".format(self.n_attack_data))
    ...
    def cnt_attack_data_sample(self):
        normal_users = 0
        total_train = []
        for user in self.users:
            if normal_users < self.n_users:
                total_train.append(user.n_train_data_samples)
            normal_users += 1
        avg_train = int(sum(total_train) / len(total_train))
    ...
    def send_parameters(self):
        ...
        for user in self.users:
            ...
            elif normal_users >= self.n_users:
                user.set_poison_parameters(self.model)
            normal_users += 1
...
################# weight parameters update #################
    ...
    def aggregate_parameters(self):
        ...
        # avg_total_train = int(total_train / self.n_users)       # 计算平均的数据量用来给攻击用户做FedAvg
        ratio_list = []
        for user in self.selected_users:
            if user.n_train_data_samples == 0:      # 攻击用户
                self.add_parameters(user, self.n_attack_train_data_samples / total_train)
                ratio_list.append(self.n_attack_train_data_samples / total_train)
            else:

...
class BST(nn.Module):
    def __init__(self, users, ratings, movies, genres, device, args=None):
        ...
        self.linear = nn.Sequential(#nn.Linear(1024, 1024), nn.BatchNorm1d(1024), nn.LeakyReLU(), nn.Dropout(0.2),
                                    # nn.Linear(256, 512), nn.BatchNorm1d(512), nn.LeakyReLU(), nn.Dropout(0.2),
                                    nn.Linear(256, 128), nn.BatchNorm1d(128), nn.LeakyReLU(), nn.Dropout(0.2),
                                    nn.Linear(128, 1), )
    ...
    def forward(self, x):
        ...
        output = self.linear(img_features)
